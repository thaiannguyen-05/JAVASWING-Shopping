package Controller;

import Model.Supplier;
import View.add;
import View.customerView;
import View.supplierView;
import View.findForm;
import View.update;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.Vector;
import javax.swing.JFrame;
import javax.swing.JOptionPane;
import javax.swing.JTable;
import javax.swing.table.DefaultTableModel;

public class controllerSupplier {

    private Supplier supplier;
    private supplierView supplierView;
    private add addView;
    private update updateView;
    private findForm findForm;
    public static int selectSup;
    
    public controllerSupplier(supplierView supplierView) throws SQLException {
        this.supplier = new Supplier();
        this.supplierView = supplierView;
    }

    public controllerSupplier() {
    }

    public void handleAddButton() throws SQLException {
        addView = new add();
        addView.setControllerSupplierView(this);
        addView.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        addView.setVisible(true);
    }

    public void handleUpdateButton() throws SQLException {
        updateView = new update();
        updateView.setControllerSupplier(this);
        selectSup = supplierView.getSelectRow();
        updateView.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        updateView.setVisible(true);
    }

    public void handleFindButton() throws SQLException {
        findForm = new findForm(supplierView);
        findForm.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        findForm.setVisible(true);
    }

    public void handleDeleteButton() throws SQLException {
        int choice = JOptionPane.showConfirmDialog(null,
                "Are you sure you want to delete this employee?",
                "Confirm Deletion",
                JOptionPane.YES_NO_OPTION);
        boolean confirmed = (choice == JOptionPane.YES_OPTION);

        int row = supplierView.getTable().getSelectedRow();
        String x = String.valueOf(supplierView.getTable().getValueAt(row, 0));

        if (confirmed) {
            String sql = "DELETE FROM supplier WHERE sID = ?";

            try (PreparedStatement pr = db.mycon().prepareStatement(sql)) {
                pr.setString(1, x);
                pr.executeUpdate();
                tableLoading();
                calTotalBudget();
            }
        }
    }

    public void tableLoading() throws SQLException {

        DefaultTableModel dt = (DefaultTableModel) supplierView.getTable().getModel();
        dt.setRowCount(0);

        Vector<String> List;
        List = supplier.getData();

        int rowCount = List.size() / 10; // Số cột là 9
        for (int i = 0; i < rowCount; i++) {
            Vector<Object> row = new Vector<>();
            for (int j = 0; j < 10; j++) {
                row.add(List.get(i * 10 + j));
            }
            dt.addRow(row);
        }
    }

    public void handleKeyrelease() throws SQLException {
        String name = supplierView.getNameFind();

        String sql = "SELECT * FROM supplier WHERE Name LIKE ?";
        try (PreparedStatement pr = db.mycon().prepareStatement(sql)) {
            pr.setString(1, "%" + name + "%");

            DefaultTableModel dt = (DefaultTableModel) supplierView.getTable().getModel();
            dt.setRowCount(0);
            ResultSet rs = pr.executeQuery();

            while (rs.next()) {
                Vector v = new Vector();

                v.add(rs.getString("sID"));
                v.add(rs.getString("Name"));
                v.add(rs.getString("Address"));
                v.add(rs.getString("Gender"));
                v.add(rs.getString("Email"));
                v.add(rs.getString("Dob"));
                v.add(rs.getString("UserName"));
                v.add(rs.getString("PassWord"));
                v.add(rs.getString("Phone"));
                v.add(rs.getString("Budget"));

                dt.addRow(v);
            }
        } catch (SQLException e) {
            tableLoading();
        }
    }

    public Vector<String> getDataUserSelect() {
        int row = supplierView.getSelectRow();
        JTable customerTable = supplierView.getTable();

        Vector<String> list = new Vector<>();
        list.add((String) customerTable.getValueAt(row, 0));
        list.add((String) customerTable.getValueAt(row, 1));
        list.add((String) customerTable.getValueAt(row, 2));
        list.add((String) customerTable.getValueAt(row, 3));
        list.add((String) customerTable.getValueAt(row, 4));
        list.add((String) customerTable.getValueAt(row, 5));
        list.add((String) customerTable.getValueAt(row, 6));
        list.add((String) customerTable.getValueAt(row, 7));
        list.add((String) customerTable.getValueAt(row, 8));

        return list;
    }

    public void calTotalBudget() {
        JTable supplierTable = supplierView.getTable();
        int countRow = supplierTable.getRowCount();
        double totalBudget = 0;

        for (int i = 0; i < countRow; i++) {
            Double value = Double.valueOf(supplierTable.getValueAt(i, 9).toString());
            totalBudget += value;
        }

        java.text.DecimalFormat df = new java.text.DecimalFormat("#,###.##");
        supplierView.getBudgetTotal().setText(df.format(totalBudget));
    }
}
